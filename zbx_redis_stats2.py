...
import sys, redis, json, re, struct, time, socket, argparse, os
...
parser.add_argument('-l','--list',dest='list_port',action='store',help='list redis port',default=None)
...
def list_ports():
	#t=os.popen("""sudo netstat -natp|awk -F: '/redis-server/&&/LISTEN/{print $2}'|awk '{print $1}' """)
	t=os.popen("""ss -ltp | awk -F: '/redis-server/&&/LISTEN/{print $2}'|awk '{print $1}' """)
	ports = []
